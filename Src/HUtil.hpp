...
#define _ps1  std::cout << "--------------------------------\n";
#define _ps2  std::cout << "----------------------------------------------------------------\n";
#define _ps3  std::cout << "------------------------------------------------------------------------------------------------\n";
#define _pse1  std::cout<< "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n";
#define _pse2  std::cout<< "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n";
#define _pse3  std::cout<< "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n";
#define _plr(a)   _pl(KLRED,a, KNRM);
#define _ply(a)   _pl(KLYEL,a, KNRM);
#define _plb(a)   _pl(KLBLU,a, KNRM);
#define _plm(a)   _pl(KLMAG,a, KNRM);
#define _plg(a)   _pl(KLGRN,a, KNRM);
#define _plc(a)   _pl(KLCYN,a, KNRM);

#define _plrb(a)   _pl(BRED,a, BNRM);
#define _plyb(a)   _pl(BYEL,a, BNRM);
#define _plbb(a)   _pl(BBLU,a, BNRM);
#define _plmb(a)   _pl(BMAG,a, BNRM);
#define _plgb(a)   _pl(BGRN,a, BNRM);
#define _plcb(a)   _pl(BCYN,a, BNRM);
           
#define _pr(a)   _p(KLRED,a, KNRM);
#define _py(a)   _p(KLYEL,a, KNRM);
#define _pb(a)   _p(KLBLU,a, KNRM);
#define _pm(a)   _p(KLMAG,a, KNRM);
#define _pg(a)   _p(KLGRN,a, KNRM);
#define _pc(a)   _p(KLCYN,a, KNRM);

#define _prb(a)   _p(BRED,a, BNRM);
#define _pyb(a)   _p(BYEL,a, BNRM);
#define _pbb(a)   _p(BBLU,a, BNRM);
#define _pmb(a)   _p(BMAG,a, BNRM);
#define _pgb(a)   _p(BGRN,a, BNRM);
#define _pcb(a)   _p(BCYN,a, BNRM);

template <typename... Args>
std::string _fprint(std::string_view rt_fmt_str, Args&&... args) {
...
public:
    HTimer(int i = 0):timer_number(i) { reset(); }
    inline void reset() { start_time = std::chrono::steady_clock::now();}
    
	inline void stop() {elapsed_time = std::chrono::steady_clock::now();}
    
    void p() {
        double tmp = e() / 1.0e6;
		std::cout << ">>> Timer [" << timer_number << "] = " << tmp << " s\n";
    }
    
    void  pr() {
        double tmp = er() / 1.0e6;
        std::cout << ">>> Timer [" << timer_number << "] = " << tmp << " us\n";
 ...
 //   }
	...
	inline i64  e() { 
        ...
        return std::chrono::duration_cast<std::chrono::microseconds>(elapsed_time - start_time).count();
    }
  
	inline i64  er() {
		elapsed_time = std::chrono::steady_clock::now();
        std::chrono::time_point<std::chrono::steady_clock> tmp = start_time;
        reset();
		return std::chrono::duration_cast<std::chrono::microseconds>(elapsed_time - tmp).count();
	}
...
public:
    ...
    bool compareLineStrict(std::string cmp_str) {
		if (line == cmp_str) {
			return true;
		}
		return false;
	...
	bool compareLine(std::string cmp_str) {
		if (str2upper(trim(line)) == str2upper(trim(cmp_str))) {
			return true;
		}
		return false;
	}
	...
	int parseLineD(std::vector<double>& v, char separator = ' ')
	{
		int c = 0;
        v.clear();
			std::istringstream iss(line);
	...
	int parseLineI(std::vector<int>& v, char separator = ' ')
	{
		int c = 0;
        v.clear();
		std::istringstream iss(line);
